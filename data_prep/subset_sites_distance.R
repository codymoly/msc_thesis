# RLS SUBSET 30km
## select sites with a minimum distance of 30km
## note that there will be still several surveys with the same coordinates
...
# filter observations > 2019-12-31, because we have some missing SST data for 2009 (envPred needs 10 years)
...
###### subset survey sites based on respective distance
...
nrow(unique(rls_dist["family"])) # 79
nrow(unique(rls_dist["species_name"])) # 928
...
## number of families and species
nrow(unique(rls_dist["family"])) # 83
nrow(unique(rls_dist["species_name"])) # 1004
