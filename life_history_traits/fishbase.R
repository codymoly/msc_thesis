...
# get fb traits Rhinecanthus rectangulus
...
# maturity_traits = maturity(
#   species_list = species_vec,
#   fields = NULL,
#   server = getOption("FISHBASE_API", "fishbase"))
# 
# maturity_traits_sub = maturity_traits %>% select(Species, Sex, starts_with(c("AgeMat", "LengthMat")))
# 
# larvae_maturity = left_join(larvae_traits_sub, maturity_traits_sub, by = "Species")
# 
# larvae_maturity_avg = larvae_maturity %>% 
#   group_by(Species) %>% 
...
missing_species = setdiff(species_df, fb_traits_compl["valid_name"])

# save data
## fb traits
write.csv(fb_traits_compl,"~/projects/msc_thesis/data/fishbase_bodysize.csv", row.names = FALSE)
write.csv(fb_traits_compl,"/media/mari/Crucial X8/fishbase_bodysize.csv", row.names = FALSE)
# ## missing species
