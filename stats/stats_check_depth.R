...
coords_10 = readr::read_delim("/media/mari/Crucial X8/coords_10km.csv", delim = ",")
...
# join both datasets
eco_sst_10 = dplyr::left_join(coords_10, eco_sst, by = c("latitude", "longitude"))

# let's have look if the data is grouped by area...
if (plot_by_area == TRUE) {
  cwm_area = ggplot(data = eco_sst_10, aes(x = sst_env_col, y = bodysize_cwm_total, colour = area)) + 
    geom_point() +
    theme(legend.position="none")
  cwv_area = ggplot(data = eco_sst_10, aes(x = sst_env_col, y = bodysize_cwv_total, colour = area)) + 
    geom_point() +
    theme(legend.position="none")
  richn_area = ggplot(data = eco_sst_10, aes(x = sst_env_col, y = sp_richness, colour = area)) + 
    geom_point()
...
# fill values
eco_env_out = eco_sst_10 # save copy
...
hist(na.omit(eco_env$sst_raw_var))
...
summary(model15) # plot seas and col, 
...
cwv110 = model.sub.cwv[[1]]
cwv4718  = model.sub.cwv[[2]]
anova(cwv110, cwv4718)
