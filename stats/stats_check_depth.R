...
eco_env_shallow = eco_env %>% 
  dplyr::filter(depth_bin == "(0,10]")

eco_env_deep = eco_env %>% 
  dplyr::filter(depth_bin == "(10,20]")
...
## arrange by site
eco_env_shallow = eco_env_shallow %>% 
...
eco_env_shallow$new_survey_id = as.character(eco_env_shallow$new_survey_id)
## add s for survey to each element
eco_env_shallow$new_survey_id = paste("S", eco_env_shallow$new_survey_id, sep="")
...
eco_env_binned = eco_env_shallow %>% 
  mutate(lon_bin = cut(longitude, breaks = (168-113)/0.25),
         lat_bin = cut(latitude, breaks = (168-113)/0.25)
  )
