...
# import datasets
eco_data = readr::read_delim("/media/mari/Crucial X8/cwm_10m_data.csv", delim = ",")
...
hist(na.omit(eco_env$size_class_cwv))
...
eco_env = eco_env %>% 
  dplyr::mutate(sp_richness_sqrt = sqrt(sp_richness),
...
## standardise 
final_sites[6:32] <- lapply(final_sites[6:32], function(x) c(scale(x)))
...
anova(best_fit.sc.cwm, best_fit.sc.cwm.2)
### summarise best fitting model
